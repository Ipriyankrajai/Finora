  ...
  describe("summary", () => {
    it("calculates monthly summary correctly", async () => {
      ...
      const mockTransactions = [
        { type: "INCOME" as const, amountCents: BigInt(100000) }, // $1000
        { type: "INCOME" as const, amountCents: BigInt(50000) }, // $500
        { type: "EXPENSE" as const, amountCents: BigInt(20000) }, // $200
        { type: "EXPENSE" as const, amountCents: BigInt(30000) }, // $300
        { type: "EXPENSE" as const, amountCents: BigInt(10000) }, // $100
      ];
      ...
      expect(result.monthlySummary.incomeCents).toBe(BigInt(150000));
      // Expense: $200 + $300 + $100 = $600 = 60000 cents
      expect(result.monthlySummary.expenseCents).toBe(BigInt(60000));
      // Net: $1500 - $600 = $900 = 90000 cents
      expect(result.monthlySummary.netCents).toBe(BigInt(90000));
    });
    ...
    it("returns top 5 tags with Other grouping", async () => {
      ...
      const mockTagSpending = [
        ...
        },
        {
          ...
          tagColor: "#0000FF",
          totalCents: BigInt(30000),
        },
        {
          tagId: MOCK_TAG_ID_4,
          tagName: "Utilities",
          tagColor: "#FFFF00",
          totalCents: BigInt(20000),
        },
        {
          tagId: MOCK_TAG_ID_5,
          tagName: "Shopping",
      ...
      expect(result.otherTagsTotal).toBe(BigInt(15000));
    });
    ...
    it("returns loan overview with projections", async () => {
      ...
      const mockLoans = [
        {
          ...
          termMonths: 60,
          monthlyPaymentCents: BigInt(19333), // ~$193.33
          currencyCode: "USD",
          startDate: new Date("2024-01-01"),
          createdAt: new Date(),
          updatedAt: new Date(),
          payments: [
            { principalCents: BigInt(15000), interestCents: BigInt(5000) },
            { principalCents: BigInt(15500), interestCents: BigInt(4500) },
          ],
        },
        {
          id: MOCK_LOAN_ID_2,
          ...
          principalCents: BigInt(500000), // $5,000
          annualRatePercent: 8.0,
          termMonths: 24,
          monthlyPaymentCents: BigInt(22610), // ~$226.10
          currencyCode: "USD",
      ...
      expect(result.loanOverview[0]!.balanceCents).toBe(BigInt(969500));
      ...
      expect(result.loanOverview[0]!.totalInterestRemainingCents).toBeGreaterThanOrEqual(0n);

      // Second loan: balance = 500000 (no payments)
      expect(result.loanOverview[1]!.name).toBe("Personal Loan");
      expect(result.loanOverview[1]!.balanceCents).toBe(BigInt(500000));
    ...
    it("calls projectPayoff with correct balance", async () => {
      ...
      const mockLoans = [
        {
          ...
          termMonths: 60,
          monthlyPaymentCents: BigInt(18871), // ~$188.71 (standard PMT)
          currencyCode: "USD",
          startDate: new Date("2024-01-01"),
          createdAt: new Date(),
          updatedAt: new Date(),
          payments: [
            { principalCents: BigInt(100000), interestCents: BigInt(4167) }, // $1000 principal
          ],
        },
      ];
      vi.mocked(prisma.transaction.findMany).mockResolvedValue([]);
      ...
      expect(result.loanOverview[0]!.balanceCents).toBe(BigInt(900000));
    ...
    it("handles loan with zero balance", async () => {
      ...
      const mockLoans = [
        {
          ...
          name: "Paid Off Loan",
          interestType: "COMPOUND" as const,
          principalCents: BigInt(100000), // $1,000
          annualRatePercent: 5.0,
          termMonths: 12,
          ...
          updatedAt: new Date(),
          payments: [
            { principalCents: BigInt(100000), interestCents: BigInt(2500) }, // Full principal paid
          ],
        },
      ];
      vi.mocked(prisma.transaction.findMany).mockResolvedValue([]);
      ...
      expect(result.loanOverview[0]!.totalInterestRemainingCents).toBe(BigInt(0));
    });
    ...
    it("handles negative net (expenses exceed income)", async () => {
      ...
      const mockTransactions = [
        { type: "INCOME" as const, amountCents: BigInt(50000) }, // $500
        { type: "EXPENSE" as const, amountCents: BigInt(100000) }, // $1000
      ];
      vi.mocked(prisma.transaction.findMany).mockResolvedValue(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        mockTransactions as any
      ...
      expect(result.monthlySummary.netCents).toBe(BigInt(-50000));
    });
  });
});
