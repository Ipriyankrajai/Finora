...
describe("transactionRouter", () => {
  ...
  describe("getById", () => {
    it("returns transaction for owner", async () => {
      ...
      const caller = createCaller();
      const result = await caller.transaction.getById({ id: mockTransaction.id });
  ...
  describe("create", () => {
    ...
    it("creates with multiple tags", async () => {
      ...
      const mockTransaction = createMockTransaction({
        tags: tagIds.map((tagId) => ({ tagId, tag: { id: tagId, name: "Test" } })),
      });
    ...
    it("converts amount string to cents correctly", async () => {
      ...
      expect((createCalledWith as { data: { amountCents: bigint } }).data.amountCents).toBe(12345n);
    });
  ...
  describe("update", () => {
    it("updates transaction with valid input", async () => {
      ...
      const updatedTransaction = { ...existingTransaction, description: "Updated" };
    ...
    it("replaces tags when tagIds provided", async () => {
      ...
      expect(deleteCalledWith).toEqual({ where: { transactionId: existingTransaction.id } });
  ...
  describe("delete", () => {
    it("deletes transaction", async () => {
      ...
      const caller = createCaller();
      const result = await caller.transaction.delete({ id: existingTransaction.id });
