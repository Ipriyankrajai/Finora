...
describe("loan router", () => {
      ...
      const mockLoans = [
        {
          ...mockLoan,
          payments: [
            { principalCents: BigInt(10000), interestCents: BigInt(5000) },
            { principalCents: BigInt(15000), interestCents: BigInt(4500) },
          ],
        },
      ];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  describe("getById", () => {
    it("returns loan with payments and projections", async () => {
      ...
      expect(result.balanceCents).toBe(BigInt(1000000) - BigInt(15000));
    ...
    it("calculates balance from payments correctly", async () => {
      ...
      expect(result.balanceCents).toBe(
        BigInt(1000000) - BigInt(15000) - BigInt(15500)
      );
    ...
    it("throws NOT_FOUND when loan does not exist", async () => {
      ...
      await expect(
        caller.loan.getById({ id: NONEXISTENT_ID })
      ).rejects.toThrow(
    ...
    it("creates loan with valid input", async () => {
      ...
      expect(prisma.loan.create).toHaveBeenCalledWith({
        data: {
          ...
          name: "Car Loan",
          interestType: "COMPOUND",
          principalCents: BigInt(1000000),
          annualRatePercent: 6.0,
          termMonths: 60,
          monthlyPaymentCents: BigInt(19333),
          startDate: new Date("2024-01-01"),
        },
      });
    });
    ...
    it("converts amount strings to cents", async () => {
      ...
      expect(prisma.loan.create).toHaveBeenCalledWith({
        data: expect.objectContaining({
          principalCents: BigInt(25000050), // $250,000.50 -> 25000050 cents
          monthlyPaymentCents: BigInt(126671), // $1,266.71 -> 126671 cents
        }),
      });
    });
  ...
  describe("addPayment", () => {
    it("adds regular payment with correct principal/interest split", async () => {
      ...
      const createdPayment = {
        ...
        principalCents: BigInt(95000), // ~$950
    ...
    it("calculates correct split for payment on $10000 balance at 6% rate", async () => {
      ...
      expect(capturedData!.principalCents).toBe(BigInt(95000)); // $950.00
    });
  });
